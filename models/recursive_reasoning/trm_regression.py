...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        if self.config.use_pretrained_embeddings:
            print(
                f"Attempting to load pre-trained embeddings from {self.config.tokenizer_name}..."
            )
            try:
                ...
                if (
                    pretrained_embeddings.shape[0] != self.config.vocab_size
                    or pretrained_embeddings.shape[1] != self.config.hidden_size
                ):
                    print(
                        f"WARNING: Size mismatch for embeddings. Pre-trained: {pretrained_embeddings.shape}, Model: ({self.config.vocab_size}, {self.config.hidden_size}). Not loading weights."
                    )
                else:
    ...
    def forward(
        ...
        output = self.regression_head(
            z_H[:, 0]
        )  # Regression output from the first token
...
class TinyRecursiveReasoningModel_ACTV1_Regression(nn.Module):
    ...
    def forward(
        ...
        # Forward inner model
        new_inner_carry, prediction, (q_halt_logits, q_continue_logits) = self.inner(
        ...
        with torch.no_grad():
            ...
            if self.training and (self.config.halt_max_steps > 1):
                ...
                halted = halted | (
                    torch.abs(
                        prediction.squeeze()
                        - new_current_data["labels"].squeeze()[:, 0]
                    )
